...
public class Partition<T,C> implements Iterable<Partition<T,C>.Element>
{
  ...
  private static String[] handleOdd(Integer[] ints)
  {
    String[] ans=new String[ints.length];
    for(int i=0;i<ans.length;i++)
    {
      ans[i]="Odd : "+ints[i]; //$NON-NLS-1$
    }
    return ans;
  }
  
  private static String[] handleEven(Integer[] ints)
  {
    String[] ans=new String[ints.length];
    for(int i=0;i<ans.length;i++)
    {
      ans[i]="Even : "+ints[i]; //$NON-NLS-1$
    }
    return ans;
  }
  ...
  public static void main(String[] args)
  {
    ...
    for(int i=0;i<arr.length;i++) arr[i]=(int)(Math.random()*20);
    
    Partition<Integer,String> part=Partition.array(arr).using(i->i%2==0?"Odd":"Even").merge(); //$NON-NLS-1$ //$NON-NLS-2$

    String[] str=new String[arr.length];
    for(Partition<Integer,String>.Element el:part.getElements())
    {
      System.out.println(el.getCategory()+":"+el.getItems().length); //$NON-NLS-1$
      if(el.getCategory().equals("Odd")) //$NON-NLS-1$
      {
        String[] s=handleOdd(el.getItems());
        el.inject(s).into(str);
      }
      if(el.getCategory().equals("Even")) //$NON-NLS-1$
      {
        String[] s=handleEven(el.getItems());
        el.inject(s).into(str);
      }
    }
